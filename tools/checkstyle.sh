...
CPP_FILES=$(find fservice -type f \( -iname "*.h" -o -iname "*.cc" -o -iname "*.cpp" -o -iname "*.c" -o -iname "*.hpp" \))
...
if [ -n "$CPP_FILES" ]; then
    tools/cppcheck/cppcheck -j $(nproc) --error-exitcode=1 --language=c++ --std=c++11 --enable=warning,style,performance,portability,information,missingInclude --template='{id}:{file}:{line} {message}' --suppressions-list=tools/cppcheck/suppressions --inline-suppr --inconclusive -I. $CPP_FILES
